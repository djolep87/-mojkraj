        ...
        @if($businesses->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                @foreach($businesses as $businessUser)
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-lg transition-shadow duration-300 overflow-hidden relative">
                        <!-- Rating Badge -->
                        @if($businessUser->total_ratings > 0)
                            <div class="absolute top-2 right-2 sm:top-3 sm:right-3 bg-gradient-to-br from-yellow-50 to-orange-50 rounded-lg px-2 py-1.5 sm:px-3 sm:py-2 shadow-xl border-2 border-yellow-400 z-10 backdrop-blur-sm hover:scale-105 transition-transform duration-200">
                                <div class="flex items-center space-x-1.5 sm:space-x-2">
                                    <div class="flex items-center space-x-0.5">
                                        ...
                                        @endphp
                                        @for($i = 1; $i <= 5; $i++)
                                            <svg class="w-3 h-3 sm:w-4 sm:h-4 {{ $avgRating >= $i ? 'text-yellow-500' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                                            </svg>
                                        @endfor
                                    </div>
                                    <div class="flex flex-col items-start border-l border-yellow-300 pl-1.5 sm:pl-2">
                                        <span class="text-sm sm:text-base font-bold text-gray-900 leading-tight">{{ number_format($businessUser->average_rating, 1) }}</span>
                                        <span class="text-xs text-gray-600 leading-tight whitespace-nowrap">{{ $businessUser->total_ratings }} {{ $businessUser->total_ratings == 1 ? 'ocena' : 'ocena' }}</span>
                                    </div>
                                </div>
                            </div>
                        @endif

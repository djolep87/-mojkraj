        ...
        <div class="p-6">
            ...
            <div class="mb-6">
                <div class="flex items-center space-x-6 text-sm text-gray-500">
                    ...
                    <button id="like-btn" class="flex items-center space-x-1 text-gray-500 hover:text-green-600 transition-colors duration-200" data-offer-id="{{ $offer->id }}" data-liked="{{ $isLiked ? 'true' : 'false' }}">
                        <svg class="w-4 h-4 {{ $isLiked ? 'fill-current text-green-500' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                        </svg>
                        <span id="likes-count">{{ $offer->likes }}</span>
                    </button>
                </div>
            </div>
    ...
    if (likeBtn) {
        likeBtn.addEventListener('click', function() {
            ...
            .then(data => {
                ...
                if (data.liked) {
                    likeBtn.querySelector('svg').classList.add('fill-current', 'text-green-500');
                    likeBtn.dataset.liked = 'true';
                } else {
                    likeBtn.querySelector('svg').classList.remove('fill-current', 'text-green-500');
